...
"git add": comando que agrega al área de preparación de nuestro repositorio un archivo o cambios que se le especifique en el código.<br>
            se lo puede complementar con un ".", es decir, "git add ." agregando todos los archivos en dónde te encuentres ubicado actualmente en la consola <span style="color: red">GitBash</span>.<br>
...
Una vez hecho esto, debemos abrir el programa y agregar nuestra carpeta RepositorioLocal al software como un nuevo Workspace para poder editar los arhivos y realizar los Add, Commit y Push todo dentro de esta misma herramienta. Para hacer esto navegamos a:
